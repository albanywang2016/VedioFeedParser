...
public class VedioFeedParser {
	...
	public Feed readFeed(String source_name, String channel, String linkURL, String previousLastUpdate,
		...
		try {
			...
			Elements elements = doc.select("li[class=item videoRanking ]");
			
			
			if(!first_time_retrieve){
				if(lastBuildDate.equalsIgnoreCase(previousLastUpdate)){
					return feed;
				}
			}
			...
			for(Element element : elements){
				...
				String pubDate = element.select("p[class=itemTime]").text();
				item.setPubDate(pubDate);
				
				Document doc2 = Jsoup.parse(element.toString());
				Elements ele1 = doc2.select("div[class=itemThumb]");
				
				String  itemLink = Const.NICOVIDEO + ele1.select("a").first().attr("href");
				
				item.setLink(retrieveImageURL(itemLink));
				//item.setLink(itemLink);
				
				Elements ele2 = doc2.select("div[class=itemThumb]");
				String imageUrl = ele2.select("img[class=jsLazyImage thumb]").attr("data-original");
				
				Elements ele3 = element.select("p[class=itemTitle ranking]");
				item.setTitle(ele3.select("a").first().attr("title"));
